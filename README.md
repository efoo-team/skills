...
scripts/check-skills.py    # CI チェック本体（frontmatter lint〔tags 必須・argument-hint 検査含む〕・名前衝突・類似 description 検出・起動契約整合・description 予算・コア公理等価）
.github/workflows/skill-checks.yml  # CI 定義（check-skills.py の6チェック + gitleaks によるシークレット検査）
...
- explicit-only スキル（`disable-model-invocation: true`）の description はコンテキストに載らない（公式仕様。このリポジトリの explicit-only 15本は消費ゼロ）
