...
このディレクトリを `output_dir` と呼ぶ。parent が後続フェーズで分類サブエージェント（デフォルト例: Oracle。以下、本書では単に Oracle と表記する）および実行ワーカーへ渡す基本入力は、この `output_dir` 配下にある basic-set（`pr-meta.json`, `reviews.jsonl`, `collection-manifest.json`）である。必要に応じて、収集時刻や manifest hash などの**収集サイクル識別子**を付加して、再収集による上書きと混同しないようにする。
...
スクリプト出力の `entries[]` を、CLI 収集済みの `reviews.jsonl` に**未収集 ID のぶんだけ**マージする。CLI が reaction から復元した `action` を持つ既存エントリを正本として優先し、新規 ID だけを追加する（スクリプト出力の `action` は一律 `pending`）。ID は双方とも GraphQL node id で揃っているため突き合わせできる。

```bash
# 未収集 ID のエントリだけを抽出（ID 配列同士で比較する）
jq -c \
...
# reviews.jsonl へ統合（以後も output_dir/reviews.jsonl を唯一の正本とする）
...
- ID 重複排除は `collect-review.sh`（収集内部）とマージ時（対 `reviews.jsonl`）の二段で行う。既存エントリの `action` は上書きしない
...
3. parent が収集ワーカーを再起動し、`gh pr-review-check` を実行して新規レビューを収集する
