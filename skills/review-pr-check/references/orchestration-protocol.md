...
**重要**: このフェーズで得られる件数は **raw 収集件数** であり、そのまま「実際に対応すべきレビュー件数」ではない。`reviews.jsonl` には `thread` / `issue_comment` / `review` が混在し、後段で `type: review` の参考情報やノイズ `issue_comment` を除外して初めて **actionable（実処理対象）件数** が確定する。
...
このディレクトリを `output_dir` と呼ぶ。parent が後続フェーズで Oracle および実行ワーカーへ渡す基本入力は、この `output_dir` 配下にある basic-set（`pr-meta.json`, `reviews.jsonl`, `collection-manifest.json`）である。必要に応じて、収集時刻や manifest hash などの**収集サイクル識別子**を付加して、再収集による上書きと混同しないようにする。

**重要**: 保存先は **`/tmp/github.com/{owner}/{repo}/pr/{pr-number}/` を正本** とする。実行中の repo root や `.sisyphus/evidence/` 配下へ収集結果を移してはならない。fallback 後も、後続フェーズが参照する唯一の正本は `output_dir/reviews.jsonl` である。
...
ISSUE_COMMENTS_SUPPLEMENT_PATH="${SUPPLEMENT_DIR}/issue-comments-supplement.jsonl"
REVIEW_THREADS_SUPPLEMENT_PATH="${SUPPLEMENT_DIR}/review-threads-supplement.jsonl"
...
- `reviews.jsonl`: 収集されたエントリ（JSONL形式、1行=1エントリ）
- `collection-manifest.json`: 収集完全性情報（下記Phase 2で詳細説明）

**件数の用語**:

- `rawOpenCount`: `reviews.jsonl` 上で `action` が `pending` / `fix` / `in_progress` のレコード数。観測指標であり、完了判定には使わない
...
Phase 2 で検証した `collection-manifest.json` の `completenessState` に基づいて処理を分岐する。**`reviews.jsonl` を単独で信頼してはならない。**
...
**STEP 2: REST API による Issue Comments 全ページ収集**

```bash
# ページネーション付きで全件取得（--paginate が Link ヘッダを追従）
gh api repos/{owner}/{repo}/issues/{pr-number}/comments \
  --paginate --slurp \
  -q '.[] | {id: .id, type: "issue_comment", action: "pending", author: .user.login, body: .body}' \
  > "${ISSUE_COMMENTS_SUPPLEMENT_PATH}"
```

**STEP 3: GraphQL API による Review Threads 全ページ収集**

⚠️ **重要な制約**: `comments(first: 50)` は各スレッドの最初の50件のみを取得します。スレッドに51件以上のコメントがある場合、ネストしたページネーションが必要です。完全性を保証するには、各スレッドごとに `comments(after: $commentsCursor)` の別カーソルループを実行してください。

```bash
# 注: このスクリプトは各スレッドの最初の50件のコメントのみを収集します。
# スレッドに51件以上のコメントがある場合、不完全なデータとなります。
# 完全な収集には per-thread comments ページネーションが追加で必要です。
...
while true; do
  ...
  " -f owner="{owner}" -f repo="{repo}" -F pr="{pr-number}" -f cursor="${CURSOR}")

  # エラー検出
  if echo "$RESULT" | jq -e '.errors' > /dev/null 2>&1; then
    echo "❌ GraphQL エラー検出: $(echo "$RESULT" | jq -c '.errors')"
    break
  fi

  # ネストした comments のページネーション警告
  if echo "$RESULT" | jq -e '.. | .comments?.pageInfo?.hasNextPage // false' | grep -q true; then
    echo "⚠️ 一部スレッドに51件以上のコメントがあります。完全な収集には per-thread ページネーションが必要です。"
  fi

  # 結果を蓄積
  echo "$RESULT" | jq -c '.data.repository.pullRequest.reviewThreads.nodes[]' >> "${REVIEW_THREADS_SUPPLEMENT_PATH}"

  # 次ページ判定（outer reviewThreads）
...
```bash
# 補完データから CLI 未収集分のみを抽出
# 改行区切り文字列への部分一致ではなく、ID 配列同士で比較する
jq -n -c \
  --slurpfile base "${RAW_REVIEWS_PATH}" \
  --slurpfile extraThreads "${REVIEW_THREADS_SUPPLEMENT_PATH}" \
  --slurpfile extraIssueComments "${ISSUE_COMMENTS_SUPPLEMENT_PATH}" '
    ($base | map(.id) | unique) as $existingIds
    | ($extraThreads + $extraIssueComments)[]
    | select(.id as $id | ($existingIds | index($id) | not))
  ' > "${FILTERED_SUPPLEMENT_PATH}"
...
# 以後の Oracle / parent / worker は常に output_dir/reviews.jsonl を正本として参照する
```

**STEP 5: プロベナンス記録**

```bash
# jq で有効な JSON として collection-manifest.json を更新
RECORDS_ADDED=$(wc -l < "${FILTERED_SUPPLEMENT_PATH}" | tr -d ' ')
TIMESTAMP=$(date -u +%Y-%m-%dT%H:%M:%SZ)

jq --arg ts "$TIMESTAMP" --argjson records "$RECORDS_ADDED" \
  '. + {
    "fallbackUsed": true,
    "fallbackCollection": {
      "triggeredAt": $ts,
      "sources": ["rest-issue-comments", "graphql-review-threads"],
...
- 補完データの ID 重複は**排除**する
- フォールバック実行事実を `collection-manifest.json` の `fallbackCollection` に記録する
...
`completenessState` が `complete` であることを確認した後、または fallback 実行後に output_dir/reviews.jsonl を追加収集済みのレビュー集合で更新した後、parent は分類担当サブエージェント（デフォルト: Oracle）に分類を依頼する。fallback 後も `completenessState` 自体は `incomplete` / `inconclusive` のまま残りうるため、その場合は **degraded collection** として Oracle/worker に明示して扱う。**parent 自身が validity 判定・ノイズ判定・グループの意味付けを行ってはならない。**

small run の Oracle 入力:

- `output_dir`
- `pr-meta.json`
- `reviews.jsonl`（fallback 実行後も output_dir/reviews.jsonl を正本として渡す）
...
- `output_dir`
- `pr-meta.json`
- `reviews.jsonl`（fallback 実行後も output_dir/reviews.jsonl を正本として参照する）
- `collection-manifest.json`
- `coarse shard` 一式
- 必要に応じて収集サイクル識別子
...
- parent の前処理は `bash` + `jq` のみで実行すること。Python スクリプトの生成・実行は禁止
...
- `reviews.jsonl` 全体、未処理一覧全体、他 packet の本文は渡してはならない
...
1. ユーザーに「AI再レビュー待機中...（10分ごとに確認）」と通知
2. 以下のコマンドを**必ず実行**して待機する:
   ```bash
   sleep 600
   ```
3. sleep完了後、parent が収集ワーカーを再起動し、`gh pr-review-check` を実行して新規レビューを収集する
4. **完全性ゲート再検証**: 新しい `collection-manifest.json` の `completenessState` を確認
5. 新規の **actionable entry** がある場合のみ、その差分を再処理する
6. Phase 8のループ判定へ進む
...
- `sleep 600` 以外の方法（ポーリングループ、待機なし即完了など）での代替は禁止
- Phase 7を「不要」と判断してスキップすることは禁止
- 最大待機時間の制限は設けない（ユーザーが中断可能）
...
RAW_OPEN_COUNT=$(jq -c 'select(.action == "pending" or .action == "fix" or .action == "in_progress")' "${RAW_REVIEWS_PATH}" | wc -l)
