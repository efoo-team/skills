...
description: ソフトウェアの機能境界・責務分離・モジュール分割・抽象化の設計判断を行うスキル。設計相談を受けたときの内部思考プロセスとして必ず参照する。「どう設計すべきか」「責務をどう分けるか」「リファクタリングしたい」「モジュール分割を考えたい」「抽象化が正しいか」「クラス設計を見直したい」「コードの見通しが悪い」「何をどこに置くべきか」といった設計構造の相談全般、設計レビュー、アーキテクチャ検討、コード構造の改善提案で使用する。
metadata:
