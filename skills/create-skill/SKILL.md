---
name: create-skill
description: Only use when the user explicitly invokes /create-skill (or $create-skill in Codex). Never auto-invoke. 新しい Agent Skill をゼロから対話的に作成するためのスキル。「スキルを作りたい」「この作業をスキル化したい」という場面で使用する。ユーザーへの質問で意図（解決したい課題・発火してほしい場面・利用範囲）を見極めてから、スキル化の要否判定、配置層（共通層/プロジェクト層）と名前の決定、SKILL.md の設計・執筆、チェッカーでの検証までを一貫して行う。スキル執筆の一般原則（progressive disclosure・description 執筆等）の正本は agent-native-project-design にあり、本スキルはそれを適用する作成ワークフローを提供する。
...
- **検証なくして完成なし**: SKILL.md を書いただけでは完成ではない。検証（`check-skills.py` PASS・発火テスト）まで、共通層の場合はさらに `README.md` の表更新までが「作成」である
...
3. **authoring-insights.md（外部知見の補遺。正本は agent-native-project-design 側）** — skill-authoring.md と同じ順でパス解決する:
...
- 共通層の一覧（このリポジトリ `skills/` 配下と `README.md` の表）・インストール済み `~/.agents/skills/`（common + external の実配布物）・`AGENTS.md`「統合しない判断の記録」節を読み、類似スキルを洗い出す
...
2. **類型判定**。スキルの4類型（reference / technique / pattern / discipline-enforcing）のどれかを判定する（authoring-insights.md §1）。類型によって Phase 4 の書き方と Phase 6 のテスト方法が変わる。
3. **重複判定**。Phase 1 で洗い出した類似スキルとの関係を決める:
   - 既存スキルで足りる → 作成せず、既存スキルの使い方を案内して終了
   - 既存スキルの改訂・拡張で足りる → 新規作成せず改訂を提案
   - 新規に作るが類似は残る → 作成し、統合しない根拠を `AGENTS.md`「統合しない判断の記録」節に登記する
...
| 名前 | kebab-case・1〜64文字。`helper` / `utils` のような曖昧語は不可。**このリポジトリ `skills/` 配下とインストール済み `~/.agents/skills/`（common + external の実配布物）に照合し、衝突・シャドウが無いこと**（プロジェクト横断の照合は行わない — プロジェクト層は各オーナー一任） |
...
registration.md の配置先別手順（§2〜§5）に従い、登録・配布設定を更新する。最低限:

- 共通層 team-owned: `README.md` の表とスキル数
- 特定エージェント限定: 上記 + `metadata.internal: true` + `setup.sh` 個別行
- external: `setup.sh` 行（SKILL.md は置かない）
- プロジェクト層: 正本 + `.claude/skills` symlink（`efoo-team/skills` 側の作業は無い）

### Phase 6: 検証と報告

1. 共通層は `python3 ~/ghq/github.com/efoo-team/skills/scripts/check-skills.py` を実行し `RESULT: PASS` を確認する。プロジェクト層は `python3 ~/ghq/github.com/efoo-team/skills/scripts/check-skills.py --only frontmatter --skills-dir <repo>/.agents/skills` で frontmatter lint を行う（いずれも cwd に依存しない絶対パスで実行する。registration.md §7 と同一）。類似度警告が出たら統合を再検討し、統合しないなら `AGENTS.md`「統合しない判断の記録」節に根拠を登記する
...
- 検証（`check-skills.py`・発火テスト）未実施のまま「完成」と報告しない
