...
1. `python3 ~/ghq/github.com/efoo-team/skills/scripts/check-skills.py` を実行し `RESULT: PASS` を確認する（プロジェクト層スキルの作成時も cwd に依存しないこの絶対パスで実行する。registration.md §7 と同一）。類似度警告が出たら統合を再検討し、統合しないなら `similar_groups` に根拠を登記する
