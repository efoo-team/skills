...
明示起動専用（auto-invoke させない）スキルは、ツールごとに仕組みが異なるため**3点セットを必ず揃える**（`check-skills.py` が manifest の `invocation: "explicit-only"` と突き合わせて検査する）:
...
explicit-only スキルの description はコンテキストに載らないため、トリガー語彙の埋め込みは不要。人間が読んで用途が分かる簡潔な説明でよい。opencode は両フィールドとも未対応（anomalyco/opencode#11972 をウォッチ。permission.skill の deny で近い挙動が得られるが非公式挙動のため採用していない）。
