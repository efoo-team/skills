...
- **短く保つこと**（目安100〜200行以下）。各行に「この行を消すと実際にミスが起きるか？」を問い、No なら削る。エージェントが既に守れている指示は削除するか機械強制に移す。
- **「コードから逆算できない決定」だけを書くこと**：推測不能なコマンド、デフォルトと異なるスタイル決定、コードに痕跡のない境界・制約、環境の癖。リポジトリ概要やディレクトリツリーの列挙は実証上効果がない。
- **手で書き、厳選すること**。LLM に生成させた指示ファイルをそのまま置くのは、書かないより悪い結果を生みうる。「書けば得」ではなく「厳選しなければ損」の非対称がある。
...
| アンチパターン | なぜ失敗するか | 是正 |
|---|---|---|
| 肥大した CLAUDE.md / AGENTS.md | 全指示が一様に無視される | 100〜200行以下に厳選 + 3層分離 |
| リポジトリ概要・ツリーの列挙 | 実証上効果がなくコストだけ増える | コードから逆算できない決定のみ書く |
| LLM 生成の指示ファイルを放置 | 成功率を下げコストを増やす | 手書きで厳選し、失敗観察で反復 |
...
- [ ] CLAUDE.md / AGENTS.md は100〜200行以下で、「コードから逆算できない決定」だけが書かれているか
