---
name: pre-define
description: ユーザーの曖昧な要望を、詳細な要件定義（/define）に渡せる具体的な入力へ洗練させるためのスキル。仕様決定や要件定義そのものは行わず、関連する既存仕様を調査したうえで平易な質問を重ね、コミュニケーションの齟齬をなくすことを目的とする。Only use when the user explicitly invokes /pre-define (or $pre-define in Codex). Never auto-invoke.
...
- serena: プロジェクト内のセマンティック検索（利用可能な場合）
