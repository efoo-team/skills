...
description: Mastra をランタイムとする AI サービスの設計・実装・リファクタリングで、agent / workflow / tool / memory / state の責務分離と、LLM に任せる範囲と決定的コードの境界を判断するスキル。『Mastra で設計して』『workflow を作って』『state に何を持つべきか』『memory や workspace が必要か』で使用し、single-agent first・workflow 最小化を適用する。現行 API 仕様の確認・検証・バージョン移行は mastra-framework-guide を使う。
metadata:
  tags: [mastra, ai-architecture, agent-design, workflow-design, responsibility-separation, anti-overengineering]
---
...
- 本スキルは **Mastra ランタイム固有層** の憲章である。`agent-harness-engineering` の適用優先順位に従い、**Mastra プロジェクトでは本憲章が一般則に優先する**。
- 本憲章が明示的に扱わない論点（停止条件・wrap-up・冪等性契約・認可の合成・キャッシュ規律・検証の証拠主義など）は、`agent-harness-engineering`（ランタイム非依存のハーネス設計一般則）と `agent-native-project-design`（Claude Code / Codex 等の既製ハーネス上で運用するリポジトリ側の設計）を補完として適用する。
- §1「設計の根本原則」は上記2スキルのコア公理を Mastra 文脈へ射影したものである。公理10箇条の全文複製は、3ファイル間の drift コストを避けるため行わない。
...
注意: 以下の機能名は 2026-07 時点の例示であり、Mastra のバージョンにより改名・統廃合・廃止されうる。名称を鵜呑みにせず、必ず**現在この repo にインストールされているバージョンの公式ドキュメント／型定義**で存在と正確な API 名を確認すること。以下は「どの capability カテゴリで簡素化できるか」の見取り図として読むこと（確定した識別子ではない）。
...
- ルーティング・監視・レビュー用途の agent には、必要なら **read-only memory** を優先すること。
- Memory を **system of record** として使わないこと。
- Memory に **シークレット・認証情報を保存しない**こと。

### 4.6 RequestContext の責務（存在する場合）

注: RequestContext は Mastra v1.0 で `RuntimeContext` から改名された名称である。pre-v1 の Mastra を使用しているリポジトリでは、旧名 `RuntimeContext` を読み替えること。
...
- ただし、判断材料になりうる内容の事前要約・間引きは、§7「オーバーエンジニアリング禁止ルール」の「まず圧縮せずに渡す」原則に従い、品質への影響を確認してから導入すること。filter / pagination は agent 側が取捨選択できる形で提供すること。
...
- RequestContext / Memory / 永続ストレージに属する値（責務は §4.5〜§4.7。state にはそれらの参照のみを置く）
- 再計算可能なデータ
- ステップ出力の重複コピー
...
memory の責務・scope（thread / resource）・read-only・禁止事項（secrets / 認証情報 / system of record）は §4.5 に集約している。以下は §4.5 に無い運用差分のみである。
...
RequestContext の責務・scope・禁止事項（state / memory への長期複製の禁止、不在時の明示引数渡し）は §4.6 に集約している。以下は §4.6 に無い運用差分のみである。

- 認可判断・テナント境界は request-scoped に処理し、RequestContext の値を認可合成（§12）の入力とすること。
...
- 認可の合成則は `agent-harness-engineering` §5 に従うこと：既定 fail-closed（認可未注入なら deny-all）／有効権限は Intersection（agent baseline ∩ 呼び出し元 ∩ 会話参加者）で Union を作らない／認可は検索・ツール実行の前に適用する（Auth Before Retrieval）。RequestContext に載る tenant / subject / 権限は、この合成の入力として扱うこと。
...
非自明な実装の前には設計メモを作成し、非自明な変更には検証根拠を含む成果物を残し、完了の定義を満たしたときのみ完了とみなすこと。

必須の設計メモ見出し（capability inventory・責務分担・却下した代替案・削除候補など9項目）、成果物一覧、完了の定義、実装前の最終確認リストは [references/design-memo-and-completion.md](references/design-memo-and-completion.md) に従うこと。**非自明な実装・変更に着手する前に必ず読むこと。**
